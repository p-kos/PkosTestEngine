...
namespace PkosTestEngine
{
  public static class SearchEngine
  {
    ...
    private static IList<string> TagName( Type T )
    {
      ...
      if ( typeof( HtmlSpan ).Name == T.Name )
      {
        return new List<string>( ) { "span" };
      }
    ...
    public static T FindControl<T>( this UITestControl control, string selector ) where T : HtmlControl, new( )
    {
      ...
      if ( selector.IndexOf( '[' ) > 0 )
      {
        T tmpControl = control.FindControls<T>( selector ).FirstOrDefault( );
        if ( tmpControl == null )
        {
    ...
    /// <summary>
    /// Return true if the control contains the text
    /// </summary>
    /// <param name="control"></param>
    /// <param name="text">Text to verify</param>
    /// <param name="matchExactly">True: match exactly whole word; false: match word as part of cell InnerText</param>
    /// <returns></returns>
    public static bool ContainsText( this HtmlCell control, string text, bool matchExactly = true )
    {
      if ( string.IsNullOrEmpty( control.InnerText ) )
      {
        return false;
      }
      if ( matchExactly && control.InnerText == text )
      {
        return true;
      }
      if ( !matchExactly && control.InnerText.Contains( text ) )
      {
        return true;
      }
      return false;
