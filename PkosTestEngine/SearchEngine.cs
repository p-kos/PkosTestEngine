...
namespace PkosTestEngine
{
  public static class SearchEngine
  {
    ...
    private static Type HtmlTypeByTagName( string tagName )
    {
      switch ( tagName )
      {
        case "div":
    ...
    public static ReadOnlyCollection<T> FindControls<T>( this UITestControl control, string selector = "" ) where T : HtmlControl, new( )
    {
      ...
      if ( selector.Contains( "[" ) )
      {
        ...
        if ( typeof( T ) != HtmlTypeByTagName( tagName ) )
        {
      ...
      // just TagName
      if ( !string.IsNullOrEmpty( selector ) && typeof( T ) != HtmlTypeByTagName( selector.Trim( ) ) )
      {
